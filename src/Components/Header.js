...
const Header = () => {
    ...
    return (
        <nav className={`${bg ? 'bg-white ' : ' bg-dark text-white'} border-1 fixed top-0 w-full z-10 transition-all duration-300`}>
            <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
                <div className='relative flex h-20 items-center'>
                    ...
                    <div className='flex ml-auto'>
                        ...
                        <div className='ml-4' onClick={() => setOpen(!open)}>
                            {!open ? <svg className={` lg:hidden h-8 cursor-pointer`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" >
                                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 9h16.5m-16.5 6.75h16.5" />
                            </svg> :
                                <svg className={` hover:rotate-180 transition duration-300 lg:hidden h-8 cursor-pointer`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            ...
            {/* Navbar Mobile */}
            <div className={`${open ? "fixed top-20 left-0 lg:hidden bg-dark h-sceen w-screen border-t border-purple-800 ease-in-out duration-300" : "fixed left-[-100%]"} `}>
